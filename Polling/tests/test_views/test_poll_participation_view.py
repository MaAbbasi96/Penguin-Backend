...
class PollManagementParticipationViewTest(APITestCase):
    def setUp(self):
        ...
        self.user_poll = UserPoll.objects.create(poll=self.poll, user=self.participated_user, choices={'option1': False})
    ...
    def test_vote_user_and_poll_exists(self):
        ...
        self.assertEqual(UserPoll.objects.get(id=self.user_poll.id).choices['option1'], True)
